...
class cets_EmbedGmaps {
	...
	$buttonshtml = $datajs = ''; // fix typo $buttonhtml -> $buttonshtml  --Eric@AQ
		foreach ( $types as $type => $strings ) {
			...
			$datajs .= '		example: "' . esc_js( $strings[3] ) . '"'; // replace deprecated escape_js() --Eric@AQ
			$datajs .= ",\n		width: " . $width . ",\n";
	...
	<?php echo $datajs; ?>
		...
		function cets_GEButtonClick( tag ) {
			...
			var buttons = { "<?php echo esc_js('Okay', 'cets_EmbedGmaps'); // replace deprecated escape_js() --Eric@AQ ?>": cets_GEButtonOkay, "<?php echo esc_js('Cancel', 'cets_EmbedGmaps'); // replace deprecated escape_js() --Eric@AQ ?>": cets_GEDialogClose };
			...
			jQuery("#cets_GE-dialog-message").html("<p>" + cets_GEData[tag]["instructions"] + "</p><p><strong><?php echo esc_js( __('Example:', 'cets_EmbedGmaps') ); // replace deprecated escape_js() --Eric@AQ ?></strong></p><p><code>" + cets_GEData[tag]["example"] + "</code></p>");
			...
			jQuery(".ui-dialog button").addClass("button").each(function(){
				if ( "<?php echo esc_js('Okay', 'cets_EmbedGmaps'); // replace deprecated escape_js() --Eric@AQ ?>" == jQuery(this).html() ) jQuery(this).addClass("button-highlighted");
			});
