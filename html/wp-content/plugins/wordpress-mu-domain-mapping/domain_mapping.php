...
function dm_handle_actions() {
	...
	} elseif( @$_GET[ 'action' ] == 'delete' ) { // suppress warning if $_GET['action'] is not set --Eric@AQ
...
function dm_manage_page() {
	...
	if ( is_array( $domains ) && !empty( $domains ) ) {
		$orig_url = parse_url( get_original_url( 'siteurl' ) );
		$domains[] = array( 'domain' => $orig_url[ 'host' ], 'path' => @$orig_url[ 'path' ], 'active' => 0 ); // suppress warning if 'path' key doesn't exist --Eric@AQ
		...
		foreach( $domains as $details ) {
			...
			if ( $details[ 'active' ] == 1 )
				echo "checked='1' ";
			echo "/>";
			$url = "{$protocol}{$details[ 'domain' ]}" . @$details[ 'path' ]; // suppress warning if 'path' key doesn't exist --Eric@AQ
...
function domain_mapping_siteurl( $setting ) {
	...
	if ( !isset( $return_url[ $wpdb->blogid ] ) ) {
		...
		if ( get_site_option( 'dm_no_primary_domain' ) == 1
		or   wp_validate_auth_cookie("", "logged_in") // don't redirect if user has a login cookie [#4416] --Eric@AQ
		) {
